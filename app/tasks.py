...
def broadcast_battle(battle_id, battle):
    r = requests.post("http://localhost:4000/fight", json=battle)
    finished = False
...
pokemon_ids = [
    ...
    10047, 10048, 10049, 10050, 10051, 10052, 10053, 10054, 10055, 10056, 10057, 10058, 10059, 10060, 10061, 10062, 10063, 10064, 10065, 10066, 10067, 10068, 10069,
    10070, 10071, 10072, 10073, 10074, 10075, 10076, 10077, 10078, 10079, 10080, 10081, 10082, 10083, 10084, 10085, 10086, 10087, 10088, 10089, 10090, 1, 2, 3, 4, 5,
...
@celery.task()
def make_random_battle():
    print('making random battle...')
    battle = {
        'location': {'lat': random.randrange(100), 'lng': random.randrange(100)},
        'start_time': (arrow.now() + timedelta(hours = 2)).isoformat(),
    }
    pokemons = random.randrange(1, 7)
    for tkey in ('team1', 'team2'):
        battle[tkey] = {}
        battle[tkey]['trainer_id'] = random.randrange(1, 16)
